...
Lemon's OpenTofu (`azurerm`). Provisions the application's own resources, per environment: its
...
Applied by Lemon's pipeline as its last step: build image → capture digest → migrate to a terminal
