...
Where OpenTofu state lives and how it is protected: the one-time bootstrap of the resources that
cannot create themselves, identity-only access to the state account, one container per
application-environment, client-side encryption of state and plans, and the rule that the
hand-created resources are adopted into code so the plan stays clean.

Established by change `platform-bootstrap-remote-state` (backlog P-01). See `D8`, `D2`, `P3`, `P5`,
`P6`, and `docs/azure-organization.md`.
